...
H10_Unchanged <- H10_Unchanged$gene[H10_Unchanged$refBreak <= 2 &
                                      H10_Unchanged$testBreak <= 2]

H85_Unchanged <- checkUpTrend(h100.trendy, h85.trendy)
H85_Unchanged <- H85_Unchanged$gene[H85_Unchanged$refBreak <= 2 &
                                      H85_Unchanged$testBreak <= 2]


#####################
## Plot Early-Peak ##
#####################
p.heat <- plot2xHeatMap(genes.EP, dat.h100, dat.h10, h100.trendy, h10.trendy, "")
p.heat <- annotate_figure(p.heat, fig.lab = "B", fig.lab.size = 14, fig.lab.face = "bold")
p.GO <- plotGO(H100_H10_AccDec_GO$accelerated$peak$topResults)
p.GO <- annotate_figure(p.GO, fig.lab = "C", fig.lab.size = 14, fig.lab.face = "bold")
...
p.peakTime <- annotate_figure(p.peakTime, fig.lab = "D", fig.lab.size = 14, fig.lab.face = "bold")
p.StartUp <- plotStartUp(H100_H10_AccDec_GO$accelerated$peak$topResults,
                         H100_H10_AccDec$accelerated$peak,
                         allGO.human, h100.trendy, h10.trendy) + theme(legend.position = "none")
p.StartUp <- annotate_figure(p.StartUp, fig.lab = "E", fig.lab.size = 14, fig.lab.face = "bold")
...
p.countPie <- annotate_figure(p.countPie, fig.lab = "A", fig.lab.size = 14, fig.lab.face = "bold")

jpeg(filename = "Trendy/TrendyFigs/EarlyPeak.jpeg", height = 10.5, width = 8, units = "in", res = 300, quality = 0.9)
grid.arrange(p.countPie, p.heat, p.GO, p.peakTime, p.StartUp,
...
## Plot Early-Up ##
###################
p.heat <- plot2xHeatMap(genes.EU, dat.h100, dat.h10, h100.trendy, h10.trendy, "", sortMax = FALSE)
p.heat <- annotate_figure(p.heat, fig.lab = "B", fig.lab.size = 14, fig.lab.face = "bold")
p.GO <- plotGO(H100_H10_AccDec_GO$accelerated$earlyUp$topResults)
p.GO <- annotate_figure(p.GO, fig.lab = "C", fig.lab.size = 14, fig.lab.face = "bold")
p.StartUp <- plotStartUp(H100_H10_AccDec_GO$accelerated$earlyUp$topResults,
                         H100_H10_AccDec$accelerated$earlyUp,
                         allGO.human, h100.trendy, h10.trendy)
p.StartUp <- annotate_figure(p.StartUp, fig.lab = "D", fig.lab.size = 14, fig.lab.face = "bold")
p.Duration <- plotUpDuration(H100_H10_AccDec_GO$accelerated$earlyUp$topResults,
                             H100_H10_AccDec$accelerated$earlyUp,
                             allGO.human, h100.trendy, h10.trendy,
                             testShiftLeft = FALSE)
p.Duration <- annotate_figure(p.Duration, fig.lab = "E", fig.lab.size = 14, fig.lab.face = "bold")
...
grid.arrange(p.countPie, p.heat, p.GO, p.StartUp, p.Duration,
             layout_matrix = rbind(c(1, 3),
                                   c(2, 3),
                                   c(2, 4),
                                   c(2, 5)),
...
jpeg(filename = "Trendy/TrendyFigs/DE.jpeg", height = 10.5, width = 8, units = "in", res = 300, quality = 0.9)
...
## Supplementary data ##
AG <- read.csv("~/Documents/Research/Chris_Mixing_2019/Final_MixData/BrainSpan/RawData/AllGenes/Expression.csv",header=FALSE,row.names=1)
AG.col <- read.csv("~/Documents/Research/Chris_Mixing_2019/Final_MixData/BrainSpan/RawData/AllGenes/Columns.csv")
AG.row <- read.csv("~/Documents/Research/Chris_Mixing_2019/Final_MixData/BrainSpan/RawData/AllGenes/Rows.csv")
AG <- AG[!duplicated(AG.row$gene_symbol), ]
rownames(AG) <- AG.row$gene_symbol[!duplicated(AG.row$gene_symbol)]

AG <- AG[, AG.col$age %in% c("8 pcw", "9 pcw", "12 pcw")]
AG.col <- AG.col[AG.col$age %in% c("8 pcw", "9 pcw", "12 pcw"), ]

AG <- do.call(cbind, lapply(sort(unique(AG.col$structure_acronym)), function(region) {
  rowMeans(AG[, AG.col$structure_acronym == region, drop = FALSE])
}))
colnames(AG) <- sort(unique(AG.col$structure_acronym))

AG.geoMean <- apply(AG, 1, function(x) {exp(mean(log(x)))})
AG <- apply(AG, 2, function(x) {x / median(x / AG.geoMean)})

AG.med <- apply(AG, 1, median)
AG.FC <- apply(AG, 2, function(x) {x / AG.med})

BS.geneList <- lapply(colnames(AG.FC), function(region) {
  rownames(AG.FC)[AG.FC[, colnames(AG.FC) == region] >= 1.25]
})
names(BS.geneList) <- colnames(AG.FC)

maxGene <- max(sapply(BS.geneList, length))
BS.geneDat <- sapply(BS.geneList, function(geneVec) {
  c(geneVec, rep("", maxGene - length(geneVec)))
})
colnames(BS.geneDat) <- names(BS.geneList)
write.csv(BS.geneDat, file = "BrainSpan/SuppBrainSpanGenes.csv")
...
p4 <- grid.arrange(h100_0$plot + theme(legend.position = "none"), 
...
jpeg(filename = "Trendy/TrendyFigs/DoseResponse.jpeg", height = 8, width = 12, units = "in", res = 300, quality = 0.9)
...
onDat <- data.frame(
  gene = H100_H10_UpDown$deUp$deOn,
  "mean (sd)" = sapply(H100_H10_UpDown$deUp$deOn, function(g) {
    paste0(
      round(mean(h10.trendy[[g]]$Fitted.Values), 1), 
      " (",
      round(sd(h10.trendy[[g]]$Fitted.Values), 1),
      ")"
    )
  }),
  "max" = sapply(H100_H10_UpDown$deUp$deOn, function(g) {
    paste0(round(max(h10.trendy[[g]]$Fitted.Values), 1))
  })
)
write.csv(onDat, file = "Paper_Figures/SuppOnGenes.csv")
# p.OnDat <- tableGrob(onDat, theme = tableTheme, rows = rep("", nrow(onDat)),
#                      cols = c("gene", "mean (sd)", "max"))
# p.OnDat <- annotate_figure(p.OnDat, fig.lab = "A: On in H10", fig.lab.size = 14, fig.lab.face = "bold")

offDat <- data.frame(
  gene = H100_H10_UpDown$deDown$deOff,
  "mean (sd)" = sapply(H100_H10_UpDown$deDown$deOff, function(g) {
    paste0(
      round(mean(dat.h10$allDay[g, ]), 1), 
      " (",
      round(sd(dat.h10$allDay[g, ]), 1),
      ")"
    )
  }),
  "max" = sapply(H100_H10_UpDown$deDown$deOff, function(g) {
    paste0(round(max(h10.trendy[[g]]$Fitted.Values), 1))
  })
)
write.csv(offDat, file = "Paper_Figures/SuppOffGenes.csv")
# p.OffDat <- tableGrob(offDat, theme = tableTheme, rows = rep("", nrow(offDat)),
#                       cols = c("gene", "mean (sd)", "max"))
# p.OffDat <- annotate_figure(p.OffDat, fig.lab = "B: Off in H10", fig.lab.size = 14, fig.lab.face = "bold")
# 
# jpeg(filename = "Trendy/TrendyFigs/OnOffTable.jpeg", height = 10, width = 8, units = "in", res = 300, quality = 0.9)
# grid.arrange(p.OnDat, p.OffDat, nrow = 1)
# dev.off()
# 
# 
# tableTheme <- ttheme_default(
...
# )
# 
# p.OnGo <- tableGrob(H100_H10_UpDown_GO$deUp$deOn$topResults[1:10, c("Term", "Annotated", "Significant", "FDR")],
#                     rows = rep("", 10), theme = tableTheme)
# p.OnGo <- annotate_figure(p.OnGo, fig.lab = "A: Enriched 'on' genes", fig.lab.size = 14, fig.lab.face = "bold")
# 
# p.OffGo <- tableGrob(H100_H10_UpDown_GO$deDown$deOff$topResults[1:10, c("Term", "Annotated", "Significant", "FDR")],
#                      rows = rep("", 10), theme = tableTheme)
# p.OffGo <- annotate_figure(p.OffGo, fig.lab = "B: Enriched 'off' genes", fig.lab.size = 14, fig.lab.face = "bold")
# 
# jpeg(filename = "Trendy/TrendyFigs/OnOffGo.jpeg", height = 6, width = 8, units = "in", res = 300, quality = 0.9)
# grid.arrange(p.OnGo, p.OffGo, nrow = 2)
# dev.off()
...
jpeg(filename = "Trendy/TrendyFigs/SuppObsProportion.jpeg", height = 5, width = 12, 
     units = "in", res = 300, quality = 0.9)
...
p1 <- plotSlopeRatio(H100_H10_AccDec_GO$accelerated$peak$topResults,
                     ...
                     "Slope Ratio: Early Peak\n(test shift right)")
p2 <- plotUpDuration(H100_H10_AccDec_GO$accelerated$peak$topResults, 
                     H100_H10_AccDec$accelerated$peak, 
                     allGO.human, h100.trendy, h10.trendy, testShiftLeft = T, 
                     title = "Duration of up-trend: Early Peak\n(test shift left)")
h10.EP <- grid.arrange(p1, p2 + theme(strip.text.y.left = element_blank()), nrow = 1, widths = c(1, 0.8))
...
p3 <- plotSlopeRatio(H100_H10_AccDec_GO$accelerated$earlyUp$topResults,
                     ...
                     "Slope Ratio: Early Up\n(test shift left)")
p4 <- plotUpDuration(H100_H10_AccDec_GO$accelerated$earlyUp$topResults, 
                     H100_H10_AccDec$accelerated$earlyUp, 
                     allGO.human, h100.trendy, h10.trendy, testShiftLeft = F, 
                     title = "Duration of up-trend: Early Up\n(test shift right)")
h10.EU <- grid.arrange(p3, p4 + theme(strip.text.y.left = element_blank()), nrow = 1, widths = c(1, 0.8))
h10.EU <- annotate_figure(h10.EU, fig.lab = "A", fig.lab.size = 14, fig.lab.face = "bold")

jpeg(filename = "Trendy/TrendyFigs/SlopeRatios.jpeg", height = 8, width = 8, 
     units = "in", res = 300, quality = 0.9)
...
plotDat <- data.frame(
  nGenes = c(length(H100_H10_AccDec$accelerated$earlyUp), 
             length(H100_H85_AccDec$accelerated$earlyUp),
             length(H100_H10_AccDec$decelerated$lateUp),
             length(H100_H85_AccDec$decelerated$lateUp),
             length(H10_Unchanged) - 
               length(H100_H10_AccDec$accelerated$earlyUp) - 
               length(H100_H10_AccDec$decelerated$lateUp), 
             length(H85_Unchanged) - 
               length(H100_H85_AccDec$accelerated$earlyUp) - 
               length(H100_H85_AccDec$decelerated$lateUp)),
  Comparison = rep(c("Early", "Late", "Unchanged"), each = 2),
  Mixture = rep(c("H10", "H85"), 3)
)
p.EU <- ggplot(plotDat, aes(x = nGenes, y = Comparison, fill = Mixture)) + 
  theme_classic() + 
  geom_bar(stat = "identity") + 
  geom_text(aes(label = nGenes, angle = 90), size = 2.5, position = position_stack(vjust = 0.5)) + 
  labs(x = "Num. Genes", y = "Classification", title = "Changes in up-trending\ngenes") + 
  scale_fill_manual(values = exptCol) +
...
plotDat <- data.frame(
  nGenes = c(length(H100_H10_AccDec$accelerated$peak), 
             length(H100_H85_AccDec$accelerated$peak),
             length(H100_H10_AccDec$decelerated$peak),
             length(H100_H85_AccDec$decelerated$peak),
             length(checkPeak(h100.trendy, h10.trendy)$gene) - 
               length(H100_H10_AccDec$accelerated$peak) - 
               length(H100_H10_AccDec$decelerated$peak), 
             length(checkPeak(h100.trendy, h85.trendy)$gene) - 
               length(H100_H85_AccDec$accelerated$peak) - 
               length(H100_H85_AccDec$decelerated$peak)),
  Comparison = rep(c("Early", "Late", "Unchanged"), each = 2),
  Mixture = rep(c("H10", "H85"), 3)
)
p.EP <- ggplot(plotDat, aes(x = nGenes, y = Comparison, fill = Mixture)) + 
  theme_classic() + 
  geom_bar(stat = "identity") + 
  geom_text(aes(label = nGenes, angle = 90), size = 2.5, position = position_stack(vjust = 0.5)) + 
  labs(x = "Num. Genes", y = "Classification", title = "Changes in peaking\ngenes") + 
  scale_fill_manual(values = exptCol) +
  theme(plot.title=element_text(size=11,face="bold.italic"),
        axis.title.x=element_text(size=9,face="bold"),
        axis.title.y=element_text(size=9,face="bold"),
...
jpeg(filename = "Trendy/TrendyFigs/GeneClassCounts.jpeg", height = 5, width = 8, 
     units = "in", res = 300, quality = 0.9)
grid.arrange(p.EU + theme(legend.position = "none"), p.EP, nrow = 1)
dev.off()
...
jpeg(filename = "Trendy/TrendyFigs/GeneClassCounts_OnlyH10.jpeg", height = 5, width = 8, 
     units = "in", res = 300, quality = 0.9)
grid.arrange(p.EU + theme(legend.position = "none"), p.EP, nrow = 1)
dev.off()
